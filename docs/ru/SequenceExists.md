...
Так как экземпляры управляемой последовательности являются динамичными данными, с помощью этого метода можно проверить существование конкретного экземпляра по его уникальному идентификатору, что возвращает метод [CreateSequence](CreateSequence.md) при создании
