...
Так как управляемых последовательностей являются динамичными данными, с помощью этого метода можно проверить существование конкретного экземпляра по его уникальному идентификатору, что возвращает метод [**CreateSequence**](/docs/ru/CreateSequence.md) при создании
